...
export default class extends SlideA {
	...
	setPenColor (options) { this.context.strokeStyle = options; }
	setPenWidth (options) { this.context.lineWidth = options; }	
	...
	renderTools(toolsElement) {
		...
		tools.forEach(t => {
			...
			btn.textContent = t.text; btn.setAttribute('title', t.title);
			btn.style.margin = '2px'; btn.style.padding = '2px 5px'; btn.onclick = t.cb;
			this.toolsBlock.appendChild(btn);
		});
	...
	onMouseUp (event) {
		if (!this.slideDIV.onmousemove) return;
	...
	finish () { 
		this.fixate('closePath');
		this.slideDIV.onmousemove = null;        
		this.slideDIV.onmousedown = null; 
