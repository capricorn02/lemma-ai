export class SlideA {
	...
	norm (XY){ return [Math.ceil(10000 * XY[0] / this.width), Math.ceil(10000 * XY[1] / this.height)]; }
	denorm (xy) { return [Math.ceil(xy[0] * this.width / 10000), Math.ceil(xy[1] * this.height / 10000)]; }	
	...
	toolsShowHide(event) { event.preventDefault(); event.stopImmediatePropagation(); if (event.button != 2) return; this.toolsBlock.style.display = (this.toolsBlock.style.display == "none") ? "block" : "none"; }	
	start () { let date = new Date(); this.t0 = date.getTime(); }	
	fixate (action, options) {
		let command = []; let date = new Date(); let t1 = date.getTime();
		command[0] = t1 - this.t0; command[1] = action; command[2] = options;
		this.commands.push(command); this.execute(command);
	}
	finish () {}
	...
	play(){
		if (this.recordMode) return;
		if (this.index == 0) {
			let command = this.commands[this.index]; this.interval = command[0];
			this.startTime = (new Date()).getTime(); this.setTimeID = setTimeout(this.next, this.interval);
		} else {
			this.interval = this.interval - (this.stopTime - this.startTime);		
			this.startTime = (new Date()).getTime(); this.setTimeID = setTimeout(this.next, this.interval);
		}	
	}	
	pause(){ if (this.recordMode) return; clearTimeout(this.setTimeID); this.stopTime = (new Date()).getTime(); }	
	next() {
		let command = this.commands[this.index]; let t0 = command[0]; this.execute(command); this.index++;
		if (this.index < this.length) {
			let nextCommand = this.commands[this.index]; let t1 = nextCommand[0];
			this.interval = t1 - t0; this.startTime = (new Date()).getTime(); this.setTimeID = setTimeout(this.next, this.interval);
		}
	}
}